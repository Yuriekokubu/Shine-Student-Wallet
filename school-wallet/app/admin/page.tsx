...
export default function AdminPage() {
  ...
  async function loadAll() {
    ...
    if (studentsResult.error) {
      setMessage(`โหลดนักเรียนไม่สำเร็จ: ${studentsResult.error.message}`)
    }
    if (productsResult.error) {
      setMessage(`โหลดสินค้าไม่สำเร็จ: ${productsResult.error.message}`)
    }
    if (transactionsResult.error) {
      setMessage(`โหลดประวัติไม่สำเร็จ: ${transactionsResult.error.message}`)
    }
  ...
  const totalBalance = useMemo(
    () => students.filter((student) => student.active).reduce((total, student) => total + Number(student.balance), 0),
    [students],
  )

  const totalTopup = useMemo(
    () => transactions.filter((transaction) => transaction.type === 'topup').reduce((total, transaction) => total + Number(transaction.amount), 0),
    [transactions],
  )

  const totalPurchase = useMemo(
    () => transactions.filter((transaction) => transaction.type === 'purchase').reduce((total, transaction) => total + Number(transaction.amount), 0),
    [transactions],
  )
  ...
  async function addStudent() {
    ...
    if (!newName.trim() || !newCode.trim()) {
      return setMessage('กรุณากรอกชื่อและรหัสนักเรียน')
    }
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          student_code: editCode.trim(),
          class_name: editClass.trim() || null,
          photo_url: photoUrl,
        })
        .eq('id', edit.id)
        .select()
      ...
      setEdit(null)
      setMessage('แก้ไขข้อมูลนักเรียนเรียบร้อยแล้ว')
    } catch (error) {
  ...
  async function topup() {
    ...
    try {
      const { data, error } = await supabase.rpc('topup_student', {
        ...
        p_note: topupNote.trim() || 'เติมเงินโดย Admin',
      })
      if (error) throw error
      ...
      setTopupReference('')
      setTopupNote('')
      setMessage(`เติมเงินสำเร็จ ฿${amount.toFixed(2)}`)
  ...
  async function showQr(student: Student) {
    try {
      setQr(await QRCode.toDataURL(`SW:${student.qr_token}`, { width: 420, margin: 2 }))
    } catch (error) {
      setMessage(`สร้าง QR ไม่สำเร็จ: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  async function saveProduct() {
    ...
    const stock = Math.floor(Number(productStock))

    if (!productName.trim() || !Number.isFinite(price) || price < 0 || !Number.isInteger(stock) || stock < 0) {
      return setMessage('กรุณากรอกข้อมูลสินค้าให้ถูกต้อง')
    }
    ...
    try {
      ...
      if (productEdit) {
        const { data, error } = await supabase
          .from('products')
          .update({ name: productName.trim(), price, stock, image_url: imageUrl })
          .eq('id', productEdit.id)
          .select()
          .single()
        if (error) throw error
        ...
        const { data, error } = await supabase
          .from('products')
          .insert({ name: productName.trim(), price, stock, image_url: imageUrl })
          .select()
          .single()
        if (error) throw error
      ...
      setMessage(`บันทึกสินค้าไม่สำเร็จ: ${error instanceof Error ? error.message : String(error)}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function deleteProduct(product: Product) {
    ...
    try {
      ...
      setMessage(`ลบสินค้าไม่สำเร็จ: ${error instanceof Error ? error.message : String(error)}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (!ready || !authorized) {
    return <main className="shell"><div className="card">กำลังตรวจสอบสิทธิ์ Admin...</div></main>
  }
  ...
  return (
    <main className="shell">
      <div className="topbar admin-topbar">
        <div>
          <div className="brand">⚙️ School Wallet Admin</div>
          <div className="muted">จัดการนักเรียน · เงิน · สินค้า · ประวัติธุรกรรม</div>
        </div>
        <div className="admin-actions">
          <Link href="/kiosk" className="btn">จุดขาย</Link>
          <Link href="/" className="btn">หน้าหลัก</Link>
      ...
      {tab === 'students' ? (
              ...
              {filtered.filter((student) => student.active).map((student) => (
                <div className="student-row" key={student.id}>
                  <div className="student-main">
                    {student.photo_url ? <img src={student.photo_url} alt="" className="student-avatar" /> : <div className="student-avatar-placeholder student-avatar">👤</div>}
                    <div><b>{student.full_name}</b><div className="muted">{student.student_code} · {student.class_name || 'ไม่ระบุชั้น'}</div></div>
                  </div>
                  <div className="student-balance">฿{Number(student.balance).toFixed(2)}</div>
                  <div className="student-actions">
                    <button className="btn primary" onClick={() => openHistory(student)}>＋ เติมเงิน</button>
                    <button className="btn" onClick={() => showQr(student)}>▦ QR</button>
                    <button className="btn" onClick={() => openEdit(student)}>✎ แก้ไข</button>
                    <button className="btn danger-btn" onClick={() => deleteStudent(student)}>ปิดใช้งาน</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="card admin-section">
          ...
          <div className="product-form">
            <input className="input" placeholder="ชื่อสินค้า" value={productName} onChange={(event) => setProductName(event.target.value)} />
            <input className="input" type="number" min="0" step="0.01" placeholder="ราคา" value={productPrice} onChange={(event) => setProductPrice(event.target.value)} />
            <input className="input" type="number" min="0" step="1" placeholder="Stock" value={productStock} onChange={(event) => setProductStock(event.target.value)} />
            <label className="btn upload-btn">📷 รูปสินค้า<input type="file" accept="image/*" hidden onChange={(event) => setProductImage(event.target.files?.[0] || null)} /></label>
            <button className="btn primary" onClick={saveProduct}>{productEdit ? 'บันทึกแก้ไข' : 'เพิ่มสินค้า'}</button>
          </div>
          <div className="products admin-products">
            {products.filter((product) => product.active).map((product) => (
              <div className="product admin-product" key={product.id}>
                {product.image_url ? <img src={product.image_url} alt="" className="product-thumb" /> : <div className="product-thumb-placeholder">🛍️</div>}
                <div className="product-info"><b>{product.name}</b><div className="muted">฿{Number(product.price).toFixed(2)} · เหลือ {product.stock} ชิ้น</div></div>
                <div className="row"><button className="btn" onClick={() => editProduct(product)}>✎ แก้ไข</button><button className="btn danger-btn" onClick={() => deleteProduct(product)}>ปิดการขาย</button></div>
              </div>
            ))}
          </div>
        </div>
      ...
      {selected && (
        <div className="card admin-section history-panel">
          <div className="section-heading"><div><h2>💳 {selected.full_name}</h2><p className="muted">{selected.student_code} · {selected.class_name || '-'}</p></div><button className="btn" onClick={() => setSelected(null)}>ปิด</button></div>
          <div className="history-balance">฿{Number(selected.balance).toFixed(2)}<small>ยอดคงเหลือ</small></div>
          <div className="notice topup-box"><h3>＋ เติมเงินนักเรียน</h3><div className="row"><input className="input" type="number" min="0.01" step="0.01" placeholder="จำนวนเงิน" value={topupAmount} onChange={(event) => setTopupAmount(event.target.value)} /><input className="input" placeholder="อ้างอิง เช่น ใบเสร็จ" value={topupReference} onChange={(event) => setTopupReference(event.target.value)} /><input className="input" placeholder="หมายเหตุ" value={topupNote} onChange={(event) => setTopupNote(event.target.value)} /><button className="btn primary" onClick={topup} disabled={topupLoading}>{topupLoading ? 'กำลังเติม...' : 'เติมเงิน'}</button></div></div>
          <h3>ประวัติการเงิน</h3>
          {historyLoading ? <div className="status">กำลังโหลด...</div> : !history.length ? <div className="status">ยังไม่มีรายการ</div> : <div style={{ overflowX: 'auto' }}><table className="admin-table"><thead><tr><th>วันเวลา</th><th>รายการ</th><th>จำนวน</th><th>ยอดก่อน</th><th>ยอดหลัง</th><th>หมายเหตุ / อ้างอิง</th></tr></thead><tbody>{history.map((transaction) => <tr key={transaction.id}><td>{new Date(transaction.created_at).toLocaleString('th-TH', { dateStyle: 'medium', timeStyle: 'short' })}</td><td>{transaction.type === 'topup' ? 'เติมเงิน' : transaction.type === 'purchase' ? 'ซื้อสินค้า' : transaction.type === 'refund' ? 'คืนเงิน' : 'ปรับยอด'}</td><td className="money">{transaction.type === 'purchase' ? '-' : '+'}฿{Number(transaction.amount).toFixed(2)}</td><td>฿{Number(transaction.balance_before).toFixed(2)}</td><td>฿{Number(transaction.balance_after).toFixed(2)}</td><td>{transaction.note || '-'}{transaction.reference && <div className="muted">Ref: {transaction.reference}</div>}</td></tr>)}</tbody></table></div>}
        </div>
      )}
      ...
      {edit && (
        <div className="card admin-section">
          <div className="section-heading"><h2>✎ แก้ไขนักเรียน</h2><button className="btn" onClick={() => setEdit(null)}>ยกเลิก</button></div>
          <div className="row admin-edit-row">
            <input className="input" value={editName} onChange={(event) => setEditName(event.target.value)} placeholder="ชื่อ" />
            <input className="input" value={editCode} onChange={(event) => setEditCode(event.target.value)} placeholder="รหัส" />
            <input className="input" value={editClass} onChange={(event) => setEditClass(event.target.value)} placeholder="ชั้น" />
            <label className="btn upload-btn">📷 เปลี่ยนรูป<input type="file" accept="image/*" hidden onChange={(event) => { const file = event.target.files?.[0] || null; setEditPhoto(file); setEditPhotoPreview(file ? URL.createObjectURL(file) : edit.photo_url || '') }} /></label>
            <button className="btn" onClick={() => { setRemovePhoto(true); setEditPhoto(null); setEditPhotoPreview('') }}>ลบรูป</button>
            <button className="btn primary" onClick={saveEdit}>บันทึก</button>
          </div>
          {editPhotoPreview && <img src={editPhotoPreview} alt="preview" className="admin-preview" />}
        </div>
      )}
