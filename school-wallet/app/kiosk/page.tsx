...
export default function KioskPage() {
  ...
  const lookup = useCallback(async (token: string) => {
    if (!token.trim()) {
      setMessage('กรุณาสแกน QR หรือกรอก QR Token')
      return
    }
    ...
    try {
      ...
      if (!result.data) {
        setStudent(null)
        setMessage('ไม่พบ QR นักเรียน กรุณาลองใหม่อีกครั้ง')
        return
      }
  ...
  return (
    <main className="shell kiosk-page">
        ...
        <section className="card kiosk-scanner-card">
          <div className="kiosk-card-heading">
            ...
            <div>
              ...
              <p>หัน QR Code ให้อยู่ในกรอบเพื่อค้นหาข้อมูล</p>
            </div>
          </div>
          ...
          <div className="kiosk-divider">
            <span>หรือกรอก Token เอง</span>
          </div>

          <div className="kiosk-manual-search">
            <input
              className="input"
              placeholder="เช่น SW:xxxxxxxx"
              value={manual}
          ...
          {message && (
            <div
              className={`status ${student ? 'success' : 'error'} kiosk-message`}
            >
              {student ? '✓ ' : '⚠ '}
              {message}
            </div>
        ...
        <section className="kiosk-result-card">
          {student ? (
            <>
              ...
              <div className="kiosk-student-profile">
                {student.photo_url ? (
                  ...
                  <div className="kiosk-student-photo kiosk-student-placeholder">
                    👤
                  </div>
                )}
              ...
              {!adminLoggedIn && (
                <div className="kiosk-topup-warning">
                  🔐 การเติมเงินต้อง Login Admin ก่อน
                </div>
              )}
            ...
            <div className="kiosk-empty-state">
              ...
              <p>
                สแกน QR Code ของนักเรียนเพื่อแสดงชื่อ รูปภาพ และยอดเงินคงเหลือ
              </p>
              <div className="kiosk-tip">
                💡 เคล็ดลับ: เพิ่มความสว่างหน้าจอ QR หากสแกนยาก
              </div>
            </div>
          )}
        </section>
          ...
          >
            ...
            {!adminLoggedIn && (
              <div className="topup-result-message error">
                ...
                <Link href="/admin/login" className="topup-login-link">
                  ไปหน้า Login Admin →
                </Link>
              </div>
            )}
            ...
            <div className="topup-field">
              ...
              <div className="topup-input-wrap">
                ...
                <input
                  ...
                  disabled={!adminLoggedIn || topupLoading}
                />
              </div>
            </div>
              ...
              {TOPUP_AMOUNTS.map((amount) => (
                <button
                  ...
                  className={`topup-quick-btn ${
                    Number(topupAmount) === amount ? 'active' : ''
                  }`}
            ...
            {topupMessage && (
              <div
                className={`topup-result-message ${
                  topupSuccess ? 'success' : 'error'
                }`}
              >
            ...
            {topupSuccess ? (
              ...
              <div className="topup-modal-actions">
                ...
                <button
                  ...
                  disabled={topupLoading || !adminLoggedIn}
                >
                  {topupLoading
                    ? 'กำลังเติมเงิน...'
                    : `ยืนยันเติม ฿${Number(topupAmount || 0).toFixed(2)}`}
                </button>
              </div>
            )}
          </div>
      ...
      <style jsx>{`
        ...
        .kiosk-topup-btn:hover,
        .kiosk-continue-btn:hover {
        ...
        .topup-student-avatar img {
          width: 100%;
          height: 100%;
          object-fit: cover;
        }

        .topup-student-mini strong,
        .topup-student-mini span {
          display: block;
        }
        ...
        .topup-quick-btn:disabled {
          opacity: 0.5;
          cursor: not-allowed;
        }

        .topup-result-message {
          margin-top: 14px;
          padding: 12px 14px;
          border-radius: 14px;
          font-weight: 600;
        ...
        .topup-result-message.success {
          background: #ecfdf5;
          color: #047857;
        }

        .topup-result-message.error {
          background: #fef2f2;
          color: #b91c1c;
        }
